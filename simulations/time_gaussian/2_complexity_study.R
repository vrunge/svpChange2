                     ...
                     seed = 123)
{
  ...
  path <- file.path("simulations/time_gaussian", "complexity_exp1_nochange.csv")
                     ...
                     seed = 456)
{
  ...
  path <- file.path("simulations/time_gaussian", "complexity_exp2_vary_k.csv")
  write.csv(df, path, row.names = FALSE)
...
if (DO_PLOT)
{
  ...
  if (!exists("df1"))
  {
    path1 <- file.path("simulations/time_gaussian", "complexity_exp1_nochange.csv")
    if (file.exists(path1)) df1 <- read.csv(path1)
  }
  if (!exists("df2"))
  {
    path2 <- file.path("simulations/time_gaussian", "complexity_exp2_vary_k.csv")
  ...
  if (exists("df1") && nrow(df1) > 0)
  {
    ...
    ggsave(filename = file.path("simulations/time_gaussian", "plots", "2_exp1_time_vs_n.pdf"), plot = p1, width = 8, height = 5)
  ...
  if (exists("df2") && nrow(df2) > 0)
  {
    p2 <- ggplot(df2, aes(x = k, y = PELT, color = "PELT")) +
      ...
      theme_minimal()
    ggsave(filename = file.path("simulations/time_gaussian", "plots", "2_exp2_time_vs_k.pdf"), plot = p2, width = 8, height = 5)
  ...
  if ((exists("df1") && nrow(df1) > 0) || (exists("df2") && nrow(df2) > 0))
  {
    ...
    p3 <- ggplot(combined, aes(x = detected, y = time, color = method, fill = method)) +
      ...
      labs(x = "Detected number of changes",
           y = "Time (s, log)") +
      theme_minimal()
    ggsave(filename = file.path("simulations/time_gaussian", "plots", "2_time_vs_detected_changes.pdf"), plot = p3, width = 8, height = 5)
