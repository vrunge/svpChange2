...
plot_statistic_panels <- function(seed = 10, n = 1000, rho = 0.7,
  ...
  for (scenario in names(scenarios)) {
    y <- scenarios[[scenario]]; ex <- exact_ar1_curve(y,rho,sigma^2)
    fu <- focus::focus_offline(y, threshold=gamma, type="arp",
                               family="arp", rho=rho)
    fs <- as.numeric(fu$stat); fs[!is.finite(fs)] <- NA_real_
    plot(ex$tau, ex$statistic, type="l", main=paste(scenario,"- exact AR1"),
         xlab="candidate change-point", ylab="statistic"); abline(h=gamma,lty=2)
    abline(v=ex$tau[which.max(ex$statistic)],col=2,lty=2)
    plot(seq_along(fs), fs, type="l", main=paste(scenario,"- unified FOCuS"),
         xlab="time", ylab="statistic"); abline(h=gamma,lty=2)
