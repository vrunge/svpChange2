...
OUTPUT_EXP1 <- file.path("simulations/other_simus/time_gaussian_legacy", "complexity_allCost_exp1_nochange.csv")
OUTPUT_EXP2 <- file.path("simulations/other_simus/time_gaussian_legacy", "complexity_allCost_exp2_vary_k.csv")

PLOT_EXP1 <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_allCost_time_vs_n.pdf")
PLOT_EXP2 <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_allCost_time_vs_k.pdf")
PLOT_COMBINED <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_allCost_complexity_figure.pdf")

SVP_METHODS <- c("SVP Gaussian", "SVP Wilcoxon", "SVP Median-Mood")

################################################################################

legacy_all_cost_gaussian_threshold <- function(n)
{
  2 * log(n)
}

legacy_all_cost_wilcoxon_threshold <- function(n, nbSeg)
{
  1.5 * sqrt((n / nbSeg)^3 / 12)
}

legacy_all_cost_mood_threshold <- function(n, nbSeg, alpha = 0.01)
{
  m <- max(n / nbSeg - 1, 1)
...
generate_legacy_all_cost_signal <- function(n,
                                      k,
                                      gap = 10,
...
time_legacy_all_cost_svp <- function(data,
                         method = SVP_METHODS,
                         nbSeg = 1)
{
  ...
  elapsed <- system.time({
    obj <- tryCatch(
      switch(
        ...
        "SVP Gaussian" = SVP(
          data,
          gamma = legacy_all_cost_gaussian_threshold(n),
          test = "gaussian_mean"
        ),
        "SVP Wilcoxon" = SVP(
          data,
          gamma = legacy_all_cost_wilcoxon_threshold(n, nbSeg),
          test = "WilcoxonCost"
        ),
        "SVP Median-Mood" = SVP(
          data,
          gamma = legacy_all_cost_mood_threshold(n, nbSeg),
          test = "MedianMoodCost"
        )
      ),
      error = function(e) {
...
run_legacy_all_cost_exp1 <- function(n_values = EXP1_N_VALUES,
                     reps = EXP1_REPS,
                     methods = SVP_METHODS,
                     noise = "student",
                     seed = 123)
{
  ...
  res_list <- future_lapply(seq_len(nrow(tasks)), function(i) {
    ...
    x <- generate_legacy_all_cost_signal(
      n = task$n,
      k = 0,
      noise = noise
    )
    out <- time_legacy_all_cost_svp(x, method = task$method, nbSeg = 1)
...
run_legacy_all_cost_exp2 <- function(n = EXP2_N,
                     ...
                     seed = 456)
{
  ...
  res_list <- future_lapply(seq_len(nrow(tasks)), function(i) {
    ...
    nbSeg <- task$k + 1
    out <- time_legacy_all_cost_svp(x, method = task$method, nbSeg = nbSeg)
...
summarise_legacy_all_cost_runtime <- function(df, xvar)
{
  df %>%
...
estimate_legacy_all_cost_slopes <- function(df, xvar)
{
  df %>%
...
plot_legacy_all_cost_exp1 <- function(df1)
{
  stats <- summarise_legacy_all_cost_runtime(df1, "n")
  slopes <- estimate_legacy_all_cost_slopes(df1, "n")
...
plot_legacy_all_cost_exp2 <- function(df2)
{
  stats <- summarise_legacy_all_cost_runtime(df2, "k")
...
plot_legacy_all_cost_combined <- function(df1, df2)
{
  stats1 <- summarise_legacy_all_cost_runtime(df1, "n") %>%
    mutate(panel = "A. No-change signals: increasing n", x = n)
  stats2 <- summarise_legacy_all_cost_runtime(df2, "k") %>%
...
run_all_cost_complexity_study <- function(workers = RUN_WORKERS)
{
  ...
  df1 <- run_legacy_all_cost_exp1()

  message("[complexity_allCost] Running Experiment 2 ...")
  df2 <- run_legacy_all_cost_exp2()
                                ...
                                df2 = NULL)
{
  ...
  if (!is.null(df1) && nrow(df1) > 0 && !is.null(df2) && nrow(df2) > 0) {
    p3 <- plot_legacy_all_cost_combined(df1, df2)
