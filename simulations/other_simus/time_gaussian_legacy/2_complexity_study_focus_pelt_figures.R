...
OUTPUT_EXP1 <- file.path("simulations/other_simus/time_gaussian_legacy", "complexity_focus_pelt_exp1_nochange.csv")
OUTPUT_EXP2 <- file.path("simulations/other_simus/time_gaussian_legacy", "complexity_focus_pelt_exp2_vary_k.csv")

PLOT_EXP1 <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_focus_pelt_time_vs_n.pdf")
PLOT_EXP2 <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_focus_pelt_time_vs_k.pdf")
PLOT_DETECTED <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_focus_pelt_time_vs_detected_changes.pdf")
PLOT_COMBINED <- file.path("simulations/other_simus/time_gaussian_legacy", "plots", "2_focus_pelt_complexity_figure.pdf")

################################################################################

legacy_focus_time_pelt <- function(data, penalty)
{
  obj <- NULL
  elapsed <- system.time({
    obj <- tryCatch(
...
legacy_focus_time_svp <- function(data, gamma)
{
  obj <- NULL
  elapsed <- system.time({
    obj <- tryCatch(
...
generate_legacy_focus_signal <- function(n, k, gap = 10)
{
  seg_count <- k + 1
...
run_legacy_focus_exp1 <- function(n_values = EXP1_N_VALUES,
                     reps = EXP1_REPS,
                     penalty_large = 1000,
                     seed = 123)
{
  ...
  res_list <- future_lapply(seq_len(nrow(tasks)), function(i) {
    ...
    bind_rows(
      legacy_focus_time_pelt(x, penalty = penalty_large),
      legacy_focus_time_svp(x, gamma = penalty_large)
    ) %>%
      mutate(
...
run_legacy_focus_exp2 <- function(n = EXP2_N,
                     ...
                     seed = 456)
{
  ...
  res_list <- future_lapply(seq_len(nrow(tasks)), function(i) {
    ...
    x <- generate_legacy_focus_signal(n = n, k = task$k, gap = gap)
    penalty_svp <- 2 * penalty_pelt

    bind_rows(
      legacy_focus_time_pelt(x, penalty = penalty_pelt),
      legacy_focus_time_svp(x, gamma = penalty_svp)
    ) %>%
      mutate(
        experiment = "vary_k",
        n = n,
...
summarise_legacy_focus_runtime <- function(df, xvar)
{
  df %>%
...
estimate_legacy_focus_slopes <- function(df, xvar)
{
  df %>%
...
plot_legacy_focus_exp1 <- function(df1)
{
  stats <- summarise_legacy_focus_runtime(df1, "n")
  slopes <- estimate_legacy_focus_slopes(df1, "n")
...
plot_legacy_focus_exp2 <- function(df2)
{
  stats <- summarise_legacy_focus_runtime(df2, "k")
...
plot_legacy_focus_detected <- function(df1, df2)
{
  bind_rows(df1, df2) %>%
...
plot_legacy_focus_combined <- function(df1, df2)
{
  stats1 <- summarise_legacy_focus_runtime(df1, "n") %>%
    mutate(panel = "A. No-change signals: increasing n", x = n)
  stats2 <- summarise_legacy_focus_runtime(df2, "k") %>%
...
run_focus_pelt_complexity_study <- function(workers = RUN_WORKERS)
{
  ...
  df1 <- run_legacy_focus_exp1()

  message("[complexity_focus_pelt] Running Experiment 2 ...")
  df2 <- run_legacy_focus_exp2()
